...
import { lineConfig_ZEPHYR} from 'config/Line_Zephyr.config';
import { replyFlex, replyText, } from '../Utils/functions/replyFunction';
import { handleMenuMessage } from 'src/zephyr/zephyrHandlers/handleMenuMessage';
...
import { handlePostbackMessage } from 'src/zephyr/zephyrHandlers/handlerspostback_Auroras';
...
import { getCustomerSession,getAdminSession,clearAdminSession,clearCustomerSession } from 'src/Utils/session/userSession.store';
...
export class ZephyrService {
    ...
    constructor(
        ...
        this.client = new Client(lineConfig_ZEPHYR);
    }
    ...
    async handleMessageEvent(event: MessageEvent, destination: string): Promise<void> {
        ...
        const statusUpdated = await handleAdminStatusUpdate(this.client, replyToken, message, destination, "Zephyr", {
        ...
        if (session && !(RESET_SESSION_KEYWORDS.includes(message))) {
            await askAndAnswerQuestion(this.client, replyToken, userId, message, destination, 'Zephyr', // or dynamic brand
                {
    ...
    async handlePostbackEvent(client: Client, event: PostbackEvent, destination: string): Promise<void> {
        ...
        if (action === 'Drivetest') {
            ...
            await askAndAnswerQuestion(client, replyToken, userId, item, destination, 'Zephyr', // or dynamic brand
                {
                    sessionService: this.sessionService,
                    googleSheetsService: this.googleSheetsService,
                    hotspotService: this.hotspotService
        ...
        if (action === 'DrivetestDateTime') {
            ...
            if (event.postback?.params && 'datetime' in event.postback.params) {
                ...
                await askAndAnswerQuestion(client, replyToken, userId, formatted, destination, 'Zephyr', // or dynamic brand
                    {
                        sessionService: this.sessionService,
                        googleSheetsService: this.googleSheetsService,
                        hotspotService: this.hotspotService
    ...
    async handleLocationEvent(event: MessageEvent): Promise<void> {
        ...
        const nearbyCenters = await findNearbyServiceCenters(latitude, longitude, zephyrCenters, 200, 5);
