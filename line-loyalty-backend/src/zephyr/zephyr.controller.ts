...
@Controller('Zephyr')
export class ZephyrController {
  constructor(
    private readonly ZephyrService: ZephyrService) { }
...
const bodyWithFormattedTimestamps = {
  ...
  events: body.events?.map(event => {
    ...
    if (!lineConfig_ZEPHYR.channelSecret) {
    ...
    const isValid = line.validateSignature(
      bodyToVerify,
      lineConfig_ZEPHYR.channelSecret,
      signature,
    );
    ...
    try {
      if (body.events && body.events.length > 0) {
        for (const event of body.events) {
          await this.ZephyrService.handleEvent(event, body.destination);
        }
      }
