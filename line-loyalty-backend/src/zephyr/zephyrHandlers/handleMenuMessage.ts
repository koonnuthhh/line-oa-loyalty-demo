...
const client = new Client(lineConfig_ZEPHYR);
...
export async function handleMenuMessage(
    ...
    switch (message) {
        ...
        case 'ข้อมูลผลิตภัณฑ์': {
            const records = await fetchAirtableImages('Zephyr_car', airtable);
        ...
        //ช่อง5
        case 'โปรโมชั่น': {
             const records = await fetchAirtableImages('Zephyr_promotion', airtable);
