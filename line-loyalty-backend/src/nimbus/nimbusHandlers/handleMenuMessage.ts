...
import { nimbusdriveTest } from "../../data/ืnimbusDrivetest";

const client = new Client(lineConfig_nimbus);
...
export async function handleMenuMessage(
    ...
    switch (message) {
        ...
        case 'ข้อมูลผลิตภัณฑ์': {
            const records = await fetchAirtableImages('Nimbus_car', airtable);
        ...
        //ช่อง5
        case 'โปรโมชั่น': {
             const records = await fetchAirtableImages('Nimbus_promotion', airtable);
