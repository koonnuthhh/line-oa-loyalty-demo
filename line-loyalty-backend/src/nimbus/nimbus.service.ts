...
import { handlePostbackMessage } from 'src/nimbus/nimbusHandlers/handlerspostback_Auroras';
import { handleCustomerAccess } from 'src/hotspot/hotspotHandlers/handleCustomerAccess';
import { nimbusCenters } from 'src/data/nimbusCenter.data';
...
import { clearCustomerSession, clearAdminSession, getAdminSession, getCustomerSession } from 'src/Utils/session/userSession.store';
...
export class nimbusService {
    ...
    constructor(
        ...
        this.client = new Client(lineConfig_nimbus);
    }
    ...
    async handleMessageEvent(event: MessageEvent, destination: string): Promise<void> {
        ...
        const statusUpdated = await handleAdminStatusUpdate(this.client, replyToken, message, destination, "Nimbus", {
        ...
        if (session && !(RESET_SESSION_KEYWORDS.includes(message))) {
            await askAndAnswerQuestion(this.client, replyToken, userId, message, destination, 'Nimbus', // or dynamic brand
                {
    ...
    async handlePostbackEvent(client: Client, event: PostbackEvent, destination: string): Promise<void> {
        ...
        if (action === 'Drivetest') {
            ...
            await askAndAnswerQuestion(client, replyToken, userId, item, destination, 'Nimbus', // or dynamic brand
                {
                    sessionService: this.sessionService,
                    googleSheetsService: this.googleSheetsService,
                    hotspotService: this.hotspotService
        ...
        if (action === 'DrivetestDateTime') {
            ...
            if (event.postback?.params && 'datetime' in event.postback.params) {
                ...
                await askAndAnswerQuestion(client, replyToken, userId, formatted, destination, 'Nimbus', // or dynamic brand
                    {
                        sessionService: this.sessionService,
                        googleSheetsService: this.googleSheetsService,
                        hotspotService: this.hotspotService
    ...
    async handleLocationEvent(event: MessageEvent): Promise<void> {
        ...
        const nearbyCenters = await findNearbyServiceCenters(latitude, longitude, nimbusCenters, 200, 5);
