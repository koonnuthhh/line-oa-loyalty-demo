...
///@Controller('lineoa-nimbus')
//export class LineoaNimbusController {}
...
const bodyWithFormattedTimestamps = {
  ...
  events: body.events?.map(event => {
    ...
    if (!lineConfig_nimbus.channelSecret) {
    ...
    const isValid = line.validateSignature(
      bodyToVerify,
      lineConfig_nimbus.channelSecret,
      signature,
    );
    ...
    try {
      if (body.events && body.events.length > 0) {
        for (const event of body.events) {
          await this.nimbusService.handleEvent(event, body.destination);
        }
      }
