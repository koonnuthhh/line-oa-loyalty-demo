...
import { handleMenuMessage } from 'src/meridian/meridianHandlers/handleMenuMessage';
...
import { handlePostbackMessage } from 'src/meridian/meridianHandlers/handlerspostback_Auroras';
...
export class MeridianService {
    ...
    constructor(
        ...
        this.client = new Client(lineConfig_MERIDIAN);
    }
    ...
    async handleMessageEvent(event: MessageEvent, destination: string): Promise<void> {
        ...
        const statusUpdated = await handleAdminStatusUpdate(this.client, replyToken, message, destination, "MERIDIAN", {
        ...
        if (session && !(RESET_SESSION_KEYWORDS.includes(message))) {
            await askAndAnswerQuestion(this.client, replyToken, userId, message, destination, 'MERIDIAN', // or dynamic brand
                {
    ...
    async handlePostbackEvent(client: Client, event: PostbackEvent, destination: string): Promise<void> {
        ...
        if (action === 'Drivetest') {
            ...
            await askAndAnswerQuestion(client, replyToken, userId, item, destination, 'MERIDIAN', // or dynamic brand
                {
                    sessionService: this.sessionService,
                    googleSheetsService: this.googleSheetsService,
                    hotspotService: this.hotspotService
        ...
        if (action === 'DrivetestDateTime') {
            ...
            if (event.postback?.params && 'datetime' in event.postback.params) {
                ...
                await askAndAnswerQuestion(client, replyToken, userId, formatted, destination, 'MERIDIAN', // or dynamic brand
                    {
                        sessionService: this.sessionService,
                        googleSheetsService: this.googleSheetsService,
                        hotspotService: this.hotspotService
    ...
    async handleLocationEvent(event: MessageEvent): Promise<void> {
        ...
        const nearbyCenters = await findNearbyServiceCenters(latitude, longitude, meridianCenters, 200, 5);
