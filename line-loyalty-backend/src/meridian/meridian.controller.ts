...
export class MeridianController {
  constructor(
    private readonly MeridianService: MeridianService) { }
...
const bodyWithFormattedTimestamps = {
  ...
  events: body.events?.map(event => {
    ...
    if (!lineConfig_MERIDIAN.channelSecret) {
    ...
    const isValid = line.validateSignature(
      bodyToVerify,
      lineConfig_MERIDIAN.channelSecret,
      signature,
    );
    ...
    try {
      if (body.events && body.events.length > 0) {
        for (const event of body.events) {
          await this.MeridianService.handleEvent(event, body.destination);
        }
      }
